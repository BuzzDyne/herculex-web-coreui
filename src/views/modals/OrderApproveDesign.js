...
const OrderApproveDesign = ({ isOpen, onClose, orderData }) => {
  const { auth } = useAuth()
  ...
  const handleSubmit = () => {
    ...
    axiosPrivate
      .patch(`/api_order/id/${orderData.id}/submit_design_acc`, {
        date: null,
  ...
  return (
    <CModal alignment="center" visible={isOpen} onClose={closeSelf}>
      <CModalHeader>
        <CModalTitle>Approve Order #{orderData.id} Design?</CModalTitle>
      </CModalHeader>
      <CModalBody>
        {formSubmitErrorMsg && <CAlert color="danger">{formSubmitErrorMsg}</CAlert>}
        <p>
          Are you sure to approve designs for order <strong>#{orderData.id}</strong>?
        </p>
      </CModalBody>
      <CModalFooter>
...
export default OrderApproveDesign
