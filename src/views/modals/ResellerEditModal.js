...
const ResellerEditModal = (props) => {
  const { sellerData, isOpen, onClose } = props
  ...
  useEffect(() => {
    setFormUsernameValue(sellerData.seller_name)
    setFormPhoneValue(sellerData.seller_phone)
  }, [sellerData])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axiosPrivate.patch(`/api_orderanku/seller/id/${sellerData.id}`, payload)
    } catch (err) {
      console.error(err)
  ...
  return (
    ...
    >
      <CModalHeader>
        <CModalTitle id="VerticallyCenteredExample">Edit Reseller</CModalTitle>
      </CModalHeader>
      <CModalBody>
        {formSubmitErrorMsg && (
          <CAlert color="danger">
        ...
        <CForm className="row g-3">
          <CCol md={6}>
            <CFormInput label="ID" disabled value={sellerData.id} />
          </CCol>
          <CCol md={6}>
            <CFormInput
