...
const OrderDonePacking = ({ isOpen, onClose, orderData }) => {
  const { auth } = useAuth()
  ...
  const handleSubmit = () => {
    ...
    axiosPrivate
      .patch(`/api_order/id/${orderData.id}/submit_packing_done`, {
        date: null,
  ...
  return (
    <CModal alignment="center" visible={isOpen} onClose={closeSelf}>
      <CModalHeader>
        <CModalTitle>Packing Done for Order #{orderData.id}?</CModalTitle>
      </CModalHeader>
      <CModalBody>
        {formSubmitErrorMsg && <CAlert color="danger">{formSubmitErrorMsg}</CAlert>}
        <p>
          Are you sure the order <strong>#{orderData.id}</strong> has been packed?
        </p>
      </CModalBody>
      <CModalFooter>
...
export default OrderDonePacking
